...
async function postRoute(req: NextRequest) {
  try {
    ...
    console.log(body);
    let weatherData = {
      message: "We logged in and got the weather!",
      city: body.city || "",
      temperature: 0,
    };
    if (body.city) {
    ...
    if (body.city === "Richmond") {
      weatherData.city = "Newport News";
      if (body.day === "Monday") {
        weatherData.temperature = 73.4;
      } else {
        weatherData.temperature = 22.4;
      }
    } else {
      throw new Error();
    }
...
//We use this function to keep all of our login api functions in one nice little file
export async function GET(
  ...
  try {
    switch (params.loginRoutes) {
      case "getInfo":
        return await getMessage(req);
      case "getSomethingElse":
        return await postRoute(req);
      default:
        return NextResponse.json({ error: true });
    }
  } catch {
    NextResponse.json({ error: true });
  }
}
*/
